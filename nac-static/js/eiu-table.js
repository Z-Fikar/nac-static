...
let AndysTable = _decorate(
      ...
      d: [
        ...
        },
        {
          ...
          value: function onCellEdit({ field, value }) {
            if (this.editCell) {
              const found =
                this.tempEditRowData ||
                this.pageData.find((item) => item === this.editCell?.row);
              if (!found) return;
              const editedRow = {
                ...found,
              };
                      ...
                      ${
                        this.tableSort.direction === "asc"
                          ? y`<svg
                            ...
                            opacity="0"
                            class="margin-left-4 ${
                              this.tableSort.field === column.field
                                ? "opacity"
                                : ""
                            }"
                          >
            ...
            ${this.pageData.map(
              (item) => y`
                ...
                <tr
                  ...
                  class="table-row ${
                    this.editMode && this.selectedRow === item ? "edit" : ""
                  } ${this.selectedRow === item ? "selected" : ""}"
                >
                  ${this.columns.map(
                    (column) => y`
                      <td>
  ...
  const nacTable = document.querySelector("${CLASS_NAME}");
    ...
    `;
        ...
        },
        {
          ...
          value: function saveSelectedRow() {
            ...
            const editedRow = this.data.find(
        ...
        },
        {
          ...
          value: function onDiscardRow() {
            if (this.selectedRow) {
              const editedRow = this.pageData.find(
                (item) => item === this.selectedRow
              );
              if (editedRow) {
